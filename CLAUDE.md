...
- **New follow-on finding (not yet investigated)**: with the env vars fixed, a full `sbt application/stryker` run now completes and reports **0.0% mutation score, every one of the 72 mutants tagged `[NoCoverage]`**. This is a different, lower-severity problem than the old exception — it's report-only (`thresholds.break = 0`) and doesn't fail the build, but suggests Stryker's per-mutant coverage detection isn't matching mutants to the tests that exercise them (or that coverage on the mutated files is genuinely sparse). Worth a follow-up investigation, not urgent.
