...
- **2026-07-30 update — failure shape changed after a plugin bump**: Renovate auto-merged `sbt-stryker4s` `1.0.x` → `1.1.0` (PR #159, commit `a8e5f2d`). Re-running `sbt application/stryker` afterward no longer reaches the `InitialTestRunFailedException` stage above — it now fails earlier, during mutant file generation itself: `java.lang.StringIndexOutOfBoundsException: Range [605, 440) out of bounds for length 1374` in Stryker4s's own `stryker4s.model.SourceSplice.render` (`MutatedFile.scala:36`), right after "Setting up environment for N mutated file(s)...". This looks like a regression inside `sbt-stryker4s` 1.1.0's source-splicing logic, not the same underspecified test-runner issue — don't conflate the two when picking this back up. It also leaves a stale `application/target/stryker4s-<uuid>` sandbox dir behind on failure (delete manually; harmless if left, since `ZStreamPocSpec`'s module scan already excludes `target/`).
- **2026-09-18 update — reverted to the original failure shape after a further plugin bump and a Scala 3.9.0 bump**: `sbt-stryker4s` is now `1.1.1` (one more patch past the `1.1.0` above) and application code is on Scala 3.9.0 (was 3.3.8 when the above was written). Re-running `sbt application/stryker` no longer hits the `SourceSplice.render` `StringIndexOutOfBoundsException` — it's back to the original `InitialTestRunFailedException` shape from 2026-07-24 (compiles fine, "Creating 3 test-runners" / "Starting initial test run...", fails ~10s later with no stack trace). So `1.1.1` fixed the source-splicing regression but the underlying upstream issue ([stryker4s#226](https://github.com/stryker-mutator/stryker4s/issues/226)) is still unresolved — this repo has now hit both known failure shapes at different plugin versions. Still no working baseline. Didn't re-attempt the debug-log-level or bisection steps already ruled out on 2026-07-24.
